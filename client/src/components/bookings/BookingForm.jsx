...
const DEFAULT_CURRENCY = (import.meta.env.REACT_APP_DEFAULT_CURRENCY || "usd").toLowerCase();
...
export default function BookingForm({ car, onBooked }) {
  ...
  const { loading: bookingsLoading, error: bookingsError, message: bookingsMessage, paymentInitiating } =
    bookingsState;
  ...
  const [topErrorMessage, setTopErrorMessage] = useState(null); // friendly aggregated error banner
  const startDate = watch("startDate");
  const endDate = watch("endDate");

  // Keep calcDays aligned with backend: use Math.ceil((e - s)/MS_PER_DAY)
  ...
  const onSubmit = (data) => {
    ...
    const safeCurrency = (car?.currency || DEFAULT_CURRENCY || "usd").toString().toLowerCase();
    const safeEstimatedTotal = Number.isFinite(Number(estimatedTotal)) ? Number(estimatedTotal) : 0;
  ...
  const onInvalid = (errs) => {
    if (!errs) return;
    const messages = [];
    if (errs.startDate) messages.push(errs.startDate.message);
    if (errs.endDate) messages.push(errs.endDate.message);
    if (!messages.length) messages.push("Please correct the highlighted fields.");
    ...
    // clear top error after a short time so the banner doesn't persist forever
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      if (res.meta.requestStatus === "fulfilled") {
        ...
        const id = booking?._id ?? booking?.id ?? res.payload?.bookingId ?? null;
        if (id) {
          setBookingId(id);
  ...
  const fetchAndApplyStatus = async (id) => {
    try {
      ...
      if (res.meta.requestStatus === "fulfilled") {
        ...
        if (["approved", "cancelled", "rejected", "succeeded", "refunded"].includes(canon)) stopPollingStatus();
      }
    } catch (err) {
  ...
  const handleProceedToPayment = async () => {
    ...
    if (payAction.meta.requestStatus === "fulfilled") {
      ...
      if (orderId) {
        navigate("/payments/checkout", { state: { payment: paymentPayload, bookingId } });
      } else {
        alert(paymentPayload?.message || "Payment initiated but no PayPal order returned.");
      }
    } else {
      alert(payAction.payload || payAction.error?.message || "Failed to initiate payment.");
    }
  };
    ...
    <>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label className=" text-sm flex items-center gap-2">
              <span className="inline-flex items-center gap-1 text-slate-600"><Calendar size={14} /> Start date</span>
            </label>
            <input
              type="date"
              {...register("startDate")}
              ...
              className={`border p-2 rounded w-full ${errors.startDate ? "ring-2 ring-rose-200" : ""}`}
              min={todayISO}
            />
            {errors.startDate && (
              <div id="startDate-error" className="text-xs text-rose-600 mt-1 flex items-center gap-2">
          ...
          <div>
            <label className=" text-sm flex items-center gap-2">
              <span className="inline-flex items-center gap-1 text-slate-600"><Clock size={14} /> End date</span>
            </label>
            <input
              type="date"
              {...register("endDate")}
              ...
              className={`border p-2 rounded w-full ${errors.endDate ? "ring-2 ring-rose-200" : ""}`}
              min={startDate || todayISO}
            />
            {errors.endDate && (
              <div id="endDate-error" className="text-xs text-rose-600 mt-1 flex items-center gap-2">
          ...
          <div className="md:col-span-2">
            <label className=" text-sm flex items-center gap-2">
              <span className="inline-flex items-center gap-1 text-slate-600"><MapPin size={14} /> Pickup location</span>
            </label>
            <input {...register("pickupLocation")} className="border p-2 rounded w-full" />
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm text-slate-600">Days: <span className="font-medium">{calcDays || 0}</span></div>
            <div className="text-sm text-slate-700">
              Estimated total:{" "}
              <span className="font-semibold">{formatCurrencyValue(estimatedTotal, displayCurrency)}</span>
            </div>
            <div className="text-xs text-slate-500 mt-1">
              {bookingId && <div>Booking status: <strong>{bookingStatus || "—"}</strong></div>}
              {polling && <div className="text-xs text-amber-700">Waiting for owner response…</div>}
              {isCancelled && <div className="text-sm text-rose-600">This booking was cancelled.</div>}
              {isRejected && <div className="text-sm text-rose-600">This booking was rejected by the owner.</div>}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {!isAuthenticated ? (
              <div className="flex flex-col items-end gap-2">
                <div className="text-sm text-slate-600">Sign in to request or pay for a booking.</div>
                <Button type="button" onClick={handleSignInClick} variant="primary">Sign in</Button>
              </div>
            ) : (
              <div className="flex items-center gap-2">
                {!bookingId && (
                  <Button type="submit" disabled={bookingsLoading || initiating}>
                    {bookingsLoading || initiating ? "Processing..." : primaryButtonLabel}
                  </Button>
                )}
                ...
                {bookingId && isPending && (
                  ...
                  >
                    {bookingsLoading ? "Canceling..." : primaryButtonLabel}
                  </button>
                )}
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-md text-white ${isApproved ? "bg-indigo-600 hover:bg-indigo-700" : "bg-gray-300 cursor-not-allowed"}`}
      ...
      <Modal isOpen={confirmOpen} onClose={() => setConfirmOpen(false)} title="Confirm booking">
        <div className="space-y-3">
          ...
          <div>
            ...
            <div>
              {formatDateSafe(pendingPayload?.startDate)} → {formatDateSafe(pendingPayload?.endDate)} ({calcDays || 0} days)
            </div>
          </div>
          <div>
            <div className="text-sm text-slate-600">Pickup</div>
            <div>{pendingPayload?.pickupLocation || car?.pickupLocation || "-"}</div>
          </div>
          <div>
            <div className="text-sm text-slate-600">Estimated total</div>
            <div className="font-semibold">{formatCurrencyValue(estimatedTotal, displayCurrency)}</div>
          </div>
          <div className="flex justify-end gap-2">
            <button onClick={() => setConfirmOpen(false)} className="px-3 py-2 border rounded">Back</button>
